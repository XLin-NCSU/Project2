...
There are four roles in the clan: leader, co-leader, elder and member. Since every clan has only one leader, I would merge the leader and co-leader together as role of leader.
...
I would like to see the difference between these roles through multiple dimensions: the trophies, donations, and online frequency.

Trophies reflect the cards' level and the skills of a player. More trophies, better the player is.

Clash Royale also allows players to donate cards to clanmates in the clan. Players can request every 7 hours for cards of their choosing. They are allowed to request for any [**Common**]{style="color: gray"} or [**Rare**]{style="color: red"} card they have unlocked. On Epic Sundays, players are allowed to request [**Epic**]{style="color: blueviolet"} cards once.

To measure the online frequency, I would create a new variable to measure how many days the player were not seen online.
...
```{r}
myclan %>% group_by(role) %>% summarize(mean = mean(days), sd = sd(days), min = min(days), Q1 = quantile(days, 0.25), median = median(days), Q3 = quantile(days, 0.75), max = max(days))
```
...
From the plot we can see that there is positive linear relationship between trophies and exp level. "leader" group has the highest exp level and trophies on average following by elder and member. We can use contour plot to show the distribution as well:
...
If you win more and lose less, obviously you would get a higher win rate. But, if you are at the lower arena with lower trophies, most likely you will meet rookie opponents, which makes you much more easily to win; meanwhile if you are at the higher arena with higher trophies, you will often meet experienced veteran opponents, even professional players -- and eat a Loss.

We want to put the win rate and trophies together to see who is the best player in a clan: more trophies with higher win rate.
...
ggplot(clanmates, aes(x = best_trophies, y = win_rate)) +
  geom_point(aes(color = role)) +
  labs(title = 'best_trophies over win_rate by role') + 
  geom_text(label = name, check_overlap=T)

```

From this plot, we can see that some "member" players have win rate higher than 0.6 but all below 6000 trophies. Two players have win rate above 0.6 and above 7000 trophies.

Only one player has win rate over 0.65 and above 8000 trophies: Yes, that's me. :)
